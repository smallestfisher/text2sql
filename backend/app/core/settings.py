...
class FieldSpec:
    """Describes one Settings attribute: how it maps to an env var, its type,
    its admin-UI group, and whether it may be overridden from the runtime DB.

    ``editable`` fields are surfaced in the admin config UI and layered from the
    ``app_config`` table on top of the env baseline. Non-editable (bootstrap)
    fields are read from the environment only, because they are consumed before
    the runtime DB (and thus the override store) is available.
    """
...
FIELD_SPECS: tuple[FieldSpec, ...] = (
    # -- bootstrap (env-only): consumed before the runtime DB / override store --
    ...
    FieldSpec("runtime_database_url", "RUNTIME_DATABASE_URL", "str", "app", None, editable=False, secret=True),
    FieldSpec("semantic_asset_store", "SEMANTIC_ASSET_STORE", "str", "app", "db", editable=False),
    ...
    # -- business database (editable) --
    FieldSpec("business_database_url", "BUSINESS_DATABASE_URL", "str", "business_db", None, secret=True),
    # -- main LLM (editable) --
    ...
    # -- SQL / execution governance (editable) --
...
EDITABLE_SPECS: tuple[FieldSpec, ...] = tuple(spec for spec in FIELD_SPECS if spec.editable)
...
class Settings(BaseModel):
    ...
    @classmethod
    def build(cls, overrides: dict[str, str] | None = None) -> "Settings":
        """Construct effective settings from the env baseline, with editable
        fields optionally overridden by ``overrides`` (env-name -> raw string,
        as stored in the ``app_config`` table).

        Resolution per field: an override wins over the env value; a blank or
        missing override reverts to the env value; a blank or missing env value
        reverts to the field default. Non-editable (bootstrap) fields ignore
        overrides entirely. Raises ``ValueError`` on a value that fails type
        coercion (e.g. a non-numeric int) so bad input is rejected before it can
        reach the container.
        """
        ...
        for spec in FIELD_SPECS:
            raw: str | None = None
            if spec.editable and spec.env in overrides:
                candidate = overrides[spec.env]
                raw = candidate if candidate is not None and candidate.strip() != "" else None
            if raw is None:
        ...
        if isinstance(values.get("semantic_asset_store"), str):
            values["semantic_asset_store"] = values["semantic_asset_store"].strip().lower()
