...
class ExampleFactory:
    ...
    def normalize(self, payload: dict | ExampleTemplateRecord) -> ExampleRecord:
        ...
        subject_domain = self._subject_domain(template.subject_domain, tables)
        example_id = template.id.strip() if template.id else self._make_id(question, subject_domain)
        dimensions = self._string_list(template.dimensions)
        ...
        return ExampleRecord(
            ...
            tables=tables,
            entities=[],
            metrics=self._string_list(template.metrics),
    ...
    def _subject_domain(self, value: object, tables: list[str]) -> str:
