...
在工程实现上，更推荐采用“本地语义特征分析 + 候选分类排序 + LLM 仲裁”的方式，而不是让 LLM 脱离结构化证据重新自由分类。
...
其中建议把 `context_delta` 也纳入 LLM 仲裁范围，使模型不仅判断“是不是 follow_up”，还判断“应继承什么、替换什么、清空什么”。
...
- 使用 JWT Session 作为当前 Web 认证方式
- 当前只建模用户、角色与数据范围，不纳入组织/部门模型
- 支持管理员配置角色和数据范围
